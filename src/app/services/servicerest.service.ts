...
import { Student } from '../classes/student'; //importante importar las clases desde sus carpetas
...
export class ServicerestService {
  ...
  updateStudent(id: any, student: Student): Observable<any> {
    return this.http.put(`${URL}/users/` + id, student, this.httpHeader).pipe(
  ...
  deleteStudent(id: any): Observable<Student[]> {
    return this.http
      .delete<Student[]>(`${URL}/users/` + id, this.httpHeader)
      .pipe(
